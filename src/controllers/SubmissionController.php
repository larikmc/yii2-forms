...
public function actionView(int $id){ $model=$this->findModel($id); if($model->status===Submission::STATUS_NEW){$model->status=Submission::STATUS_VIEWED;$model->viewed_at=time();$model->save(false);} return $this->redirect(['index']); }
...
public function actionUpdateStatus(int $id,string $status){ $m=$this->findModel($id); if(isset(Submission::statuses()[$status])){$m->status=$status;$m->save(false);} return $this->redirect(['index']); }
